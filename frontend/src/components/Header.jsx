...
const eliteHeaderStyles = `
  :root {
    --primary: #96c2db; /* Updated to Blue-Grey Theme */
    --text-main: #ffffff;
  ...
  .eh-header-wrapper {
    ...
    z-index: 1000; /* Increased Z-index so it floats above the Canvas3D */
    width: 90%;
    max-width: 1100px;
  ...
  .eh-brand-icon {
    ...
    color: #000; box-shadow: 0 0 16px rgba(150, 194, 219, 0.3); /* Updated Glow RGB */
  }
  .eh-brand-text {
...
export default function Header() {
  ...
  return (
    <>
      ...
      <style dangerouslySetInnerHTML={{ __html: eliteHeaderStyles }} />
      
      {/* Floating Island Wrapper */}
      <header className={`eh-header-wrapper ${scrolled ? 'scrolled' : ''}`}>
        <div className="eh-nav-container">
          
          {/* 1. Left: Premium Logo (Added 'interactable' class) */}
          <a href="/" className="eh-brand interactable" aria-label="Home">
            ...
            {navItems.map(item => {
              ...
              return (
                <a 
                  key={item.path} 
                  href={item.path} 
                  className={`eh-link interactable ${isActive ? 'active' : ''}`}
          ...
          {/* 3. Right: Elite CTA Button (Added 'interactable' class) */}
          <a href="mailto:contact@darshprajapati.dev" className="eh-cta-btn interactable">
