...
const ProjectModal = ({ isOpen, onClose, onSave, project }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isEditMode && !file && !currentImage) {
      // In edit mode, an image is not strictly required if one already exists
      // But if there's no current image, it should be required.
      if (!currentImage) {
    ...
    try {
      ...
      if (!res.ok) throw new Error(responseData.msg || `Failed to ${isEditMode ? 'update' : 'add'} project`);
...
export default function AdminProjectsPage() {
  ...
  const handleDeleteProject = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.msg || 'Failed to delete project');

      // Remove from UI
      setProjects(projects.filter(p => p._id !== currentProject._id));
      closeModal();
    } catch (err) {
      setError(err.message); // Show error on main page
