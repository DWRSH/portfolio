...
read           {loading && <Loader2 size={16} className="animate-spin" />}
...
// (Ye component same rahega, ismein koi change nahi)
  if (!isOpen) return null;
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75 animate-fade-in p-4">
...
ds       <h1 className="text-4xl font-bold text-white">Manage Blog Posts</h1>
        <button
...
s                 )}
                  </td>
  A               <td className="p-4 font-medium text-white whitespace-nowrap">{post.title}</td>
                  {/* Hide on mobile */}
                  <td className="p-4 text-sm text-slate-400 hidden md:table-cell">/{post.slug}</td>
s                 <td className="p-4">
                    {/* Stack buttons on small screens */}
                    <div className="flex flex-col sm:flex-row gap-2 sm:gap-4">
Access                   <button onClick={() => openEditModal(post)} className="flex items-center gap-1 text-sm text-cyan-400 hover:underline">
                        <Edit size={14} /> Edit
    }                 </button>
